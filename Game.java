...
public class Game {
	...
	public final static boolean[][][] piecesRaw = { { { true } }, // the
																	// one-cell
																	// dot.
	...
	public final static Piece[] pieces = { new Piece(piecesRaw[0]), new Piece(piecesRaw[1]), new Piece(piecesRaw[2]),
			new Piece(piecesRaw[3]), new Piece(piecesRaw[4]), new Piece(piecesRaw[5]), new Piece(piecesRaw[6]),
			new Piece(piecesRaw[7]), new Piece(piecesRaw[8]), };
	...
	public int playByLinear()
	{
		do {
			...
			if(gameOver())
				break;
			int pieceIdx = pickPiece(); // assume pickPiece validates
			Coordinate spot = b.getFirstAvailibleSpot(piecesInPlay.get(pieceIdx));
			// listArr(spots);

			b.placePiece(piecesInPlay.remove(pieceIdx), spot);
	...
	/*
	 * returns TRUE if game is over, if no pieces are playable.
	 */
	...
	private int pickPiece() {
		/*
		 * for (Piece p : piecesInPlay) System.out.println(p);
		 */
