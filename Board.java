...
public class Board {
	...
	public int scanAndClear() {
		...
		return cleared;
		/*
		 * TODO: ensure that intersecting fields do not affect score in the
		 * wrong whey.
		 */
	}
			...
			for (int j = 0; j < board[i].length - piece[0].length + 1; j++) {
				...
				for (int k = 0; k < piece.length && works; k++) {
					for (int l = 0; l < piece[k].length && works; l++) {
						/*
						 * check if there is a block at this cell in piece AND
						 * the corresp. block in board is empty, OR the cell in
						 * the piece is empty...
						 */
						works = (piece[k][l] && !board[i + k][j + l]) || !piece[k][l];
						//System.out.println(k+" "+l+" "+i+" "+j+" "+works);
					}
				}
	...
	public Coordinate getFirstAvailibleSpot(Piece p)
	{
		return getFirstAvailibleSpot(p.raw);
	}
	
	public Coordinate getFirstAvailibleSpot(boolean[][] piece) {
